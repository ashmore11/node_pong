class App
	...
	Main: ->
		...
		@socket.on "user_num", ( user_num ) =>
			...
			@delay 1000, => if user_num is 2 then @tween_title_view()
		...
		@images = [
			...
			{ id:"player_2",     src:"images/paddle.png" },
	...
	handle_load_complete: =>
		...
		@totalLoaded++
		if @images.length is @totalLoaded then @add_title_view()
	...
	tween_title_view: =>
		...
		Tween.get( @title_view ).to( y: -320, 500 ).call @add_game_view()
	...
	add_game_view: ->
		...
		@socket.on "game_started", => @start_game()
